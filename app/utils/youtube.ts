...
export const YOUTUBE_DL_PROXY_URL =
  "https://youtube-dl-extract-info-hiro18181.vercel.app"; // production
// "https://youtube-dl-extract-info-hiro18181-hiogawa.vercel.app"; // staging
...
export async function fetchVideoInfoV2(videoId: string): Promise<VideoInfo> {
  const raw = await fetchVideoInfoV2Raw(videoId);
  ...
  return {
    ...
    formats: p.streamingData.adaptiveFormats.map((f) => ({
      ...
      acodec: f.mimeType.includes("audio/") ? f.mimeType.split(";")[1] : "none",
      vcodec: f.mimeType.includes("video/") ? f.mimeType.split(";")[1] : "none",
      width: f.width ?? null,
...
export async function fetchVideoInfoV2Raw(videoId: string): Promise<any> {
